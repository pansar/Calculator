...
namespace LoreSoft.MathExpressions.Tests
{
    ...
    public class MathEvaluatorTest
    {
        ...
        {
            //Round of result in both results to account for issues 
            //if Math.Sqrt returns more decimals than home made
            double expected = Math.Round(Math.Sqrt(8), 10);
            double result = Math.Round(eval.Evaluate("sqrt(8)"),10);
        ...
        [TestCase("min((1,2))")]
        //Commented out, updated NUnit does not support, rewriten evaluate
        //[ExpectedException(typeof(ParseException))]
        public void EvaluateMisplacedComma(string expr)
        {
            Assert.That(() => eval.Evaluate(expr), Throws.TypeOf<ParseException>()); 
        }
       ...
       // [Test, ExpectedException(typeof(ParseException))]
        public void EvaluateFunctionHasTooManyArguments()
        {
            ...
            Assert.That(() => eval.Evaluate("max(1,2,3,4)"), Throws.TypeOf<ParseException>()); 
        }
